...
// Generated by CoffeeScript 1.6.1
(function() {
  var Database, LocalStorageBackend, RemoteStorageBackend, WebDavBackend,
    __slice = [].slice;
  ...
  RemoteStorageBackend = (function() {

    function RemoteStorageBackend(_changeNotification, config) {
      this._changeNotification = _changeNotification;
      this._user = config.user;
      this._rs = remoteStorage;
      this._rs.access.claim('synclist', 'rw');
      this._rs.connect(this._user);
    }

    RemoteStorageBackend.prototype.list = function() {
      var _this = this;
      return this._action((function() {
        return _this._rs.get('/synclist/');
      }), function(code, data, mimetype) {
        return Utilities.setToArray(data);
      });
    };

    RemoteStorageBackend.prototype.get = function(key) {
      var _this = this;
      return this._action((function() {
        return _this._rs.get('/synclist/' + key);
      }), function(code, data, mimetype, revision) {
        return data;
      });
    };

    RemoteStorageBackend.prototype.put = function(key, data) {
      var _this = this;
      return this._action(function() {
        return _this._rs.put('/synclist/' + key, data, 'text/plain');
      });
    };

    RemoteStorageBackend.prototype._action = function(request, transform) {
      var d;
      if (transform == null) {
        transform = (function() {});
      }
      d = jQuery.Deferred();
      if (!this._rs.connected) {
        d.reject();
      } else {
        request().then((function() {
          var args;
          args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
          return d.resolve(transform.apply(null, args));
        }), function() {
          return d.reject();
        });
      }
  ...
  (typeof exports !== "undefined" && exports !== null ? exports : this).RemoteStorageBackend = RemoteStorageBackend;
  ...
  Settings = (function() {
    ...
    function Settings(_database) {
      ...
      $('#delete-1').click(function() {
        return _this._deleteStorage();
      });
      $('#settingsForStorage #type-1').change(function() {
        var s, type;
        s = $('#settingsForStorage');
        type = $('#type-1', s).val();
        $('.flexible-setting', s).hide();
        if (type === 'WebDAV') {
          $('.setting-webdav', s).show();
        }
        if (type === 'RemoteStorage') {
          $('.setting-remotestorage', s).show();
        }
        return true;
      });
    ...
    Settings.prototype._populateSettingsForStorageDialog = function(index) {
      var data, s, type, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
      s = $('#settingsForStorage');
      data = (_ref = this._storages[index]) != null ? _ref : {};
      type = (_ref1 = data.type) != null ? _ref1 : 'RemoteStorage';
      $('input', s).val('');
      ...
      $('#type-1', s).val(type);
      $('#encpassword-1').val((_ref2 = data.encpassword) != null ? _ref2 : '');
      switch (type) {
        case "RemoteStorage":
          $('#remotestorage-address', s).val((_ref3 = data.location) != null ? _ref3 : '');
          break;
        case "WebDAV":
          $('#webdav-url', s).val((_ref4 = data.location) != null ? _ref4 : '');
          $('#webdav-username', s).val((_ref5 = data.username) != null ? _ref5 : '');
          $('#webdav-password').val((_ref6 = data.password) != null ? _ref6 : '');
      }
      return $('#type-1', s).change();
    };
    ...
    Settings.prototype._deleteStorage = function() {
      var index,
        _this = this;
      index = +$('#storage-index-1').val();
      if (index === '' || index < 0) {
      ...
      return this._database.load('settings').then(function(settings) {
        try {
          settings = JSON.parse(settings);
        } catch (error) {
          return;
